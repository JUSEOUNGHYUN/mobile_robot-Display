...
def set_main_controller(controller):
    ...
    print(f"[RingoBellManager] 메인 컨트롤러 참조 설정됨: {controller is not None}")

class RingoBellManager(QObject):
    """링고벨 시리얼 통신 관리자 (싱글톤)"""
    ...
    def __init__(self):
        ...
        print(f"[RingoBellManager] 초기화: 이동 중 다른 호출벨 무시 설정 = {self.ignore_other_bell}")
    ...
    def close(self):
        """시리얼 포트 닫기"""
        try:
            print("[RingoBellManager] 리소스 정리 시작...")
            
            # 타이머 중지
            if hasattr(self, 'read_timer') and self.read_timer.isActive():
                print("[RingoBellManager] 타이머 중지 중...")
                self.read_timer.stop()
                
            # 시리얼 연결 닫기
            if self.serial_conn and self.serial_conn.is_open:
                print("[RingoBellManager] 시리얼 포트 닫는 중...")
                self.serial_conn.close()
                self.is_connected = False
                self.connection_signal.emit(False)
            ...
            print("[RingoBellManager] 리소스 정리 완료")
            return True
        except Exception as e:
            print(f"[RingoBellManager] 리소스 정리 중 오류: {e}")
            return False
    ...
    def _get_location_by_uuid(self, uuid_value):
        """UUID로 등록된 위치 데이터 찾기"""
        # 메모리에서 UUID가 등록되어 있는지 확인
        if not self.is_uuid_registered(uuid_value):
            return None
    ...
    def _move_to_location(self, location):
        """등록된 목적지로 이동 처리"""
        try:
            print(f"목적지로 이동 처리: {location['name']}")
            
            # MainController 참조 사용
            if MAIN_CONTROLLER is not None:
                # 위치 고정 상태 확인
                if (hasattr(MAIN_CONTROLLER, "arrive_controller") and 
                    MAIN_CONTROLLER.arrive_controller.is_fixed()):
                    # 위치가 고정된 경우 메시지 표시
                    # 현재 화면 가져오기
                    current_widget = MAIN_CONTROLLER.stack.currentWidget()
                    
                    print("위치 고정 상태: 새 호출벨 요청이 무시되었습니다.")
                    return False
                
                if hasattr(MAIN_CONTROLLER, "moving_controller") and MAIN_CONTROLLER.moving_controller is not None:
                    # 목적지 선택 화면으로 전환 후 이동 화면으로 전환
                    if hasattr(MAIN_CONTROLLER, "selection_controller") and MAIN_CONTROLLER.selection_controller is not None:
                        MAIN_CONTROLLER.selection_controller.show()
                        
                        # 약간의 지연 후 이동 화면으로 전환
                        QTimer.singleShot(200, lambda: MAIN_CONTROLLER.moving_controller.start_moving(location))
                        return True
            
            return False
            
        except Exception as move_err:
            print(f"이동 처리 중 오류 발생: {str(move_err)}")
            return False
    
    def _get_current_destination(self):
        """현재 이동 중인 목적지 정보 가져오기"""
        try:
            # MainController를 통해 MovingController 접근
            if MAIN_CONTROLLER and hasattr(MAIN_CONTROLLER, "moving_controller"):
                moving_controller = MAIN_CONTROLLER.moving_controller
                if moving_controller and hasattr(moving_controller, "current_destination"):
                    return moving_controller.current_destination
            
            # 접근할 수 없는 경우 None 반환
            return None
        except Exception as e:
            print(f"현재 목적지 확인 중 오류: {str(e)}")
            return None
